...
class StripedWriter {
  ...
  //add by Zuoru: For ECPipe to get the listFileName from the StripedWriter.
  ...
  StripedWriter(StripedReconstructor reconstructor, DataNode datanode,
    ...
    // add by Zuoru: For ECPipe, we just consider the single block lost
    // so the index should be 0
    this.lostFileNum = targetIndices.length;
    this.lostFileNames = new String[lostFileNum];
    LOG.info("Zuoru: lostFileNum: " + this.lostFileNum);
    ...
    for (int i = 0; i < targets.length; i++) {
      if (targetsStatus[i]) {
        ...
        try {
          ...
          //packetBuf.hashCode();
          //LOG.info("Test by Zuoru: " + packetBuf.length);
          nSuccess++;
          success = true;
        } catch (IOException e) {
  ...
  int initTargetStreams() {
    int nSuccess = 0;
    // add by Zuoru
    LOG.info("Test by Zuoru: the target.length:" + targets.length);
