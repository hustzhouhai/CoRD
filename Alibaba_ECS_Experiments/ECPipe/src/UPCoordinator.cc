...
void UPCoordinator::doProcess() 
{
    // starts the threads
    /*
    _CmdDisThr = thread([=]{this -> CmdDistributor();});
  
    _UpdReqThr = thread([=]{this -> UpdateReqHandler();});
...
void UPCoordinator::UpdateReqHandler()
{
    ...
    /*
        cmd format:
        updated stripe num (4 bytes): 1 for random, x for trace

        update number for first stripe(4 bytes): [0, k-1]
        1st update block id in stripe (4 bytes): [0, k-1]
        1st update block's offset address (4 bytes): [0, blocksize)
        1st update block's update size (4 bytes): [1, blocksize - offset_add)
        ...

        update number for second stripe(4 bytes): [0, k-1]
        ...
    */
...
void UPCoordinator::RaidUpdate(vector<UpdBlock> &upd_blk_vec)
{
    ...
    // init node cmd and send (aside from the collector)
    /*
        cmd format:
        task type (4 bytes): 1 (read + send to collector)
        cmd len(4 bytes): cmd len (skip init first and construct last)
        block id (4 bytes): 
        collector id (4 bytes): 
    */
    for (int i=0; i<upd_num; i++)
    {
    ...
    // init collector cmd and send
    /*
        cmd format:
        task type (4 bytes): 2 (recv, compute, and send to parity)
        cmd len(4 bytes): cmd len (skip init first and construct last)
        update num (4 bytes):
        1st updated block id (4 bytes): 
        coding coefficient (4*m bytes): 
        2nd updated block id (4 bytes): 
        coding coefficient (4*m bytes): 
        ...
        1st parity block id (4 bytes): 
        2nd parity block id (4 bytes): 
        ...
    */
    if (COORDINATOR_DEBUG)
    ...
    // init parity node for raid update
    /*
        cmd format:
        task type (4 bytes): 3 (read data from collector and save local)
        cmd len(4 bytes): cmd len (skip init first and construct last)
        block id (4 bytes): the parity block id itself
    */
    for (int i=0; i<ecM; i++)
    {
        task_type = 3;
...
void UPCoordinator::DeltaUpdate(vector<UpdBlock> &upd_blk_vec)
{
    ...
    // init parity node for delta update
    /*
        cmd format:
        task type (4 bytes): 5 (read data from updated data node, aggregate one parity delta, and save local)
        cmd len(4 bytes): cmd len (skip init first and construct last)
        updated data num (4 bytes): the num of recived parity delta
        block id (4 bytes): the parity block id itself
    ...
    */
    ...
    // init cmd for updated data node, compute parity delta for each parity node, and send to parity node
    /*
        cmd format:
        task type (4 bytes): 4 (read data, compute delta, and send parity)
        cmd len(4 bytes): cmd len (skip init first and construct last)
        block id (4 bytes): local data block id
        offset_add (4 bytes)
        upd_size (4 bytes)
        coding coefficient (4*m bytes): 
        1st parity block id (4 bytes): 
...
void UPCoordinator::CrdUpdate(vector<UpdBlock> &upd_blk_vec)
{
    ...
    // init node cmd and send to parity collector
    /*
        cmd format:
        task type (4 bytes): 6 (read local + compute data delta + send to parity collector)
        cmd len(4 bytes): cmd len (skip init first and construct last)
        block id (4 bytes): 
        offset_add (4 bytes)
        upd_size (4 bytes)
        parity collector id (4 bytes): 
    */
    for (int i=0; i<upd_num; i++)
    {    
        task_type = 6;
    ...
    // init parity collector cmd 
    /*
        cmd format:
        task type (4 bytes): 7 (recv data delta, compute parity delta, and send to ecM - 1 parity)
        cmd len(4 bytes): cmd len (skip init first and construct last)
        block id (4 bytes): the parity block id itself
        update num (4 bytes):

        update sub-set num (4 bytes): the num of sub-set, the same id represent the same sub-set that can be merged
        ...
        remaining 1st parity block id (4 bytes): 
        remaining 2nd parity block id (4 bytes): 
        ...
    */
    ...
    // init remaining parity block
    /*
    cmd format:
        task type (4 bytes): 8 (recv partiy delta and save)
        cmd len(4 bytes): cmd len (skip init first and construct last)

        block id (4 bytes): the parity block id itself

        update sub-set num (4 bytes)

        1st sub-set id (4 bytes)
        merge offset_add (4 bytes)
        merge upd_size (4 bytes)

        2nd sub-set id (4 bytes)
        merge offset_add (4 bytes)
        merge upd_size (4 bytes)
