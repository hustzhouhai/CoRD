...
public class Decoder {
  ...
  DecoderInputStream generateAlternateStream(FileSystem srcFs, Path srcFile,
                      ...
                      Context context, boolean useECPipe) {   // ECPipe Integrate
    ...
    DecoderInputStream decoderInputStream = new DecoderInputStream(
        ...
        lostBlock, si, recoverFromStripeInfo, useECPipe); // ECPipe Integrate
  ...
  // ECPipe Integrate start
  public void recoverThroughECPipe(File localBlockFile, Block lostBlock,
      long blockSize) throws IOException {
    LOG.info("Decoder.recoverThroughECPipe");
    long read1=0, read2=0, readtime=0;
    long writelocal1=0, writelocal2=0, writelocaltime=0;
    OutputStream out = null;
    ECPipeInputStream ecpipeStream = null;
    try {
      out = new FileOutputStream(localBlockFile);
      ecpipeStream = new ECPipeInputStream(lostBlock.getBlockName(), conf);
      long written = 0;
      while(written < blockSize) {
	byte[] buffer = new byte[bufSize];
	read1=System.currentTimeMillis();
	ecpipeStream.read(buffer, 0, bufSize);
	read2=System.currentTimeMillis();
	readtime+=read2-read1;
	writelocal1=System.currentTimeMillis();
	out.write(buffer, 0, bufSize);
	writelocal2=System.currentTimeMillis();
	writelocaltime+=writelocal2-writelocal1;
	written += bufSize;
      }
    } catch (Exception e) {
      System.out.println("Decoder.recoverThroughECPipe.exception");
    } finally {
      LOG.info("recoverThroughECPipe.read function time = " + readtime);
      LOG.info("writelocalfile time = " + writelocaltime);
      if (out != null) {
    	out.close();
      }
    }
  }	    
  // ECPipe Integrate end
  ...
  public class DecoderInputStream extends InputStream {
    ...
    // ECPipe Integrate start
    private boolean useecpipe = false;
    private ECPipeInputStream ecpipestream = null;
	...
	Block lostBlock, StripeInfo si,
	boolean recoverFromStripeInfo, boolean useECPipe) {
	...
	this.useecpipe = true;
	try{
	  this.ecpipestream = new ECPipeInputStream(lostBlock.getBlockName(), conf);
	} catch(Exception e) {
	  System.out.println("DecoderInputStream.create.ECPipeInputStream.error!");
 	}
      }
    }
    // ECPipe Integrate end
    ...
    // ECPipe Integrate start
    private void init(boolean useECPipe) throws IOException {
      if(startTime == -1) {
	startTime = System.currentTimeMillis();
      } 
	...
	buffer = null;
      ...
      if(null == buffer) {
	bufferLen = Math.min(bufSize, limit - streamOffset);
	buffer = new byte[(int)bufferLen];
	this.ecpipestream.read(buffer, 0, (int)bufferLen);
