...
int
KfsClient::EnumerateBlocks(
    int fileId, chunkOff_t fileSize, BlockInfos& res, bool getChunkSizesFlag)
{
    return mImpl->EnumerateBlocks(fileId, fileSize, res, getChunkSizesFlag);
}
...
int
KfsClientImpl::EnumerateBlocks(
    int fileId, chunkOff_t fileSize, KfsClient::BlockInfos& res, bool getChunkSizesFlag)
{
    QCStMutexLocker l(mMutex);
    ...
    GetLayout(lop);
    if (lop.status < 0) {
        KFS_LOG_STREAM_ERROR << "get layout failed on file ID: " << fileId << " "
             << ErrorCodeToStr(lop.status) <<
        KFS_LOG_EOM;
        return lop.status;
    }
