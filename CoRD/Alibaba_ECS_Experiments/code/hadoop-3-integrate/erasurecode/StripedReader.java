...
class StripedReader {
  ...
  private void initReaders() throws IOException {
    ...
    int nSuccess = 0;
    // add by Zuoru: ECPipe
    // For the case of ECPipe, to save time, we just need to start just one reader to maintain Datachecksum
    LOG.info("Test by Zuoru: isECPipe:" + isECPipe);
    if (isECPipe){
